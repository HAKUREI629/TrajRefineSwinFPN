...
def evaluate_trace(result, theta_true, threshold):
    """
    评估 DOA 估计轨迹的连续性和准确性
    参数:
...
def batch_evaluate(input_dir, truth_dir, output_file, single_truth=False, threshold=1.0):
    ...
    for input_path, truth_path in pbar:
        ...
        result = np.load(str(input_dir) + '/' + name + '_track.npy')
        
        # 执行评估
        track_result, ospa, rmse, pd, pfa = evaluate_trace(result, theta_true, threshold)  # 需要实现evaluate_trace
