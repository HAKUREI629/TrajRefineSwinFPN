...
def evaluate_trace(btr, theta_true, threshold):
    ...
    # 如果 squeeze 后变成 1D，就把它当作 N×1 的二维数组
    if result.ndim == 1:
    ...
    return result, ospa_final, rmse_final, Pd[0][0], PFA[0][0]
...
def batch_evaluate(input_dir, truth_dir, output_file, single_truth=False, threshold=1.0):
    ...
    for input_path, truth_path in pbar:
        ...
        # 执行评估
        track_result, ospa, rmse, pd, pfa = evaluate_trace(btr, theta_true, threshold)  # 需要实现evaluate_trace
...
if __name__ == "__main__":
    ...
    parser.add_argument('--thr_min',  type=float, default=-1.0, help='阈值最小值')
    parser.add_argument('--thr_max',  type=float, default=-0.1, help='阈值最大值')
    parser.add_argument('--thr_step', type=float, default=0.1, help='阈值步长')
    ...
    file_pairs = match_input_truth(input_dir, truth_path, truth_path if args.single else None)
    ...
    for thr in thresholds:
        # 创建以阈值命名的文件夹
        thr_folder = input_dir / f"threshold_{thr:.2f}"
        thr_folder.mkdir(exist_ok=True)

        pds, pfas = [], []
        print(f"\n=== Evaluating threshold = {thr:.2f} ===")
        pbar = tqdm(file_pairs.items(), desc=f"Thr={thr:.2f}")
        for input_path, truth_file in pbar:
            btr, theta_true = load_single_pair(input_path, truth_file)
            # 调用 evaluate_trace，拿回 result 以保存
            result_array, ospa, rmse, pd, pfa = evaluate_trace(btr, theta_true, thr)
            
            # 保存该阈值下的跟踪结果
            out_name = thr_folder / f"{input_path.stem}_track.npy"
            np.save(out_name, result_array)

            pds.append(pd)
            pfas.append(pfa)

            pbar.set_postfix_str(f"{input_path.name}/{truth_file.name}:PFA={pfa:.2f}, Pd={pd:.2f}")

        if pds:
            pd_list.append(np.mean(pds))
        else:
            pd_list.append(0)

        if pfas:
            pfa_list.append(np.mean(pfas))
        else:
            pfa_list.append(0)

    # 最后保存各阈值的 Pd/PFA 曲线
    np.save(input_dir / 'thresholds.npy', thresholds)
    np.save(input_dir / 'pd_vs_threshold_exlow.npy', np.array(pd_list))
    np.save(input_dir / 'pfa_vs_threshold_exlow.npy', np.array(pfa_list))

    print("\nSaved per-threshold folders with track results,")
    print("and summary arrays thresholds.npy, pd_vs_threshold.npy, pfa_vs_threshold.npy.")
