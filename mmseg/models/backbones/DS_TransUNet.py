...
class DS_SwinUNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.encoder2 = SwinTransformer(depths=depths, num_heads=num_heads, drop_path_rate=drop_path_rate, patch_size=8, embed_dim=96)
        self.encoder.init_weights('ckpt/swin_base_patch4_window7_224_22k.pth')
        self.encoder2.init_weights('ckpt/swin_tiny_patch4_window7_224_22k.pth')
        self.layer1 = Swin_Decoder(8*embed_dim, 2, 8)
        self.layer2 = Swin_Decoder(4*embed_dim, 2, 4)
        self.layer3 = Swin_Decoder(2*embed_dim, 2, 2)
        self.layer4 = Decoder(embed_dim, embed_dim, embed_dim//2)
        self.layer5 = Decoder(embed_dim//2, embed_dim//2, embed_dim//4)
        self.down1 = nn.Conv2d(in_chans, embed_dim//4, kernel_size=1, stride=1, padding=0)
        self.down2 = conv_block(embed_dim//4, embed_dim//2)
        # self.final = nn.Conv2d(embed_dim//4, n_class, kernel_size=1, stride=1, padding=0)

        # self.loss1 = nn.Sequential(
        #         nn.Conv2d(embed_dim*8, n_class, kernel_size=1, stride=1, padding=0),
        #         nn.ReLU(),
        #         nn.Upsample(scale_factor=32)
        # )

        # self.loss2 = nn.Sequential(
        #         nn.Conv2d(embed_dim, n_class, kernel_size=1, stride=1, padding=0),
        #         nn.ReLU(),
        ...
        tb = dim_s + dim_l
        self.change1 = Conv_block(tb, embed_dim)
        self.change2 = Conv_block(tb*2, embed_dim*2)
        self.change3 = Conv_block(tb*4, embed_dim*4)
        self.change4 = Conv_block(tb*8, embed_dim*8)
