...
class ShallowExtractor(nn.Module):
    ...
    """
    def __init__(self, in_chans, out_chans, patch_size, norm_layer=None):
        ...
        self.conv1 = nn.Conv2d(in_chans, out_chans, kernel_size=patch_size, stride=patch_size)

        # if patch_size[0] > 1:
        #     self.proj = nn.Conv2d(out_chans, out_chans, kernel_size=patch_size, stride=patch_size)
        #     self.act = nn.ReLU()
        # else:
        #     self.proj = None
        #     self.act = None
    ...
    def forward(self, x):
        ...
        x = self.conv1(x)
        # x = self.act(x) if self.act is not None else x  # B C Wh Ww
        # x = self.proj(x1) if self.proj is not None else x1  # B C Wh Ww 

        if self.norm is not None:
            Wh, Ww = x.size(2), x.size(3)
            x = x.flatten(2).transpose(1, 2)
            x = self.norm(x)
            x = x.transpose(1, 2).view(-1, self.out_chans, Wh, Ww)
...
class EncoderLayer(nn.Module):
    ...
    def __init__(
        ...
        self.across_cascade_proj = nn.Linear(dim * num_cas, dim * num_cas) if num_cas!=1 else nn.Identity()
        
        # build blocks
        self.blocks = nn.Sequential(*[
            NSTB(
                dim=dim * num_cas, training_num_patches=training_num_patches, ngram=ngram, num_heads=num_heads,
        ...
        if downsample is not None:
            self.downsample = downsample(dim=dim * num_cas, downsample_dim=downsample_dim * num_cas, norm_layer=norm_layer)
        else:
...
class NGSwinTransformer(nn.Module):
    """
    Args:
    ...
    #     window_size=8, mlp_ratio=2., qkv_bias=True, img_norm=True,
    ...
    def __init__(self,
        ...
        super(NGSwinTransformer, self).__init__()
        ...
        mlp_ratio = to_ntuple(self.num_encoder_stages)(mlp_ratio)
        dpr = [x.item() for x in torch.linspace(0, drop_path_rate, sum(depths))]  # stochastic depth decay rule
        
        # Main Branch
        self.shallow_extract = ShallowExtractor(in_chans, embed_dim, patch_size, norm_layer)
        ...
        num_features = [int(embed_dim * 2 ** i) for i in range(self.num_encoder_stages)]
        ...
        # # Reconstruction
        # self.to_target = Reconstruction(target_mode, dec_dim, 1)
...
#             self.init_weights(weight_init)
    ...
    def forward_encoder(self, x):
        ...
        for i in range(self.num_encoder_stages):
            ...
            if i == 0:
                c, num_patches = rearrange(x, 'b c h w -> b (h w) c'), (H,W) # [B, HW, 1D], (H, W). ASPC
                e_, e, num_patches_l = layer(c, num_patches) # [B, HW, D], [B, HW/(2^2), D], (H/2, W/2). encoder-stage1
            else:
                c = torch.cat([self.inter_pool(c, num_patches), e], dim=-1) # [B, HW/(2^2), 2D], (H, W). ASPC
                num_patches = num_patches_l
                e_, e, num_patches_l = layer(c, num_patches_l) # [B, HW/(2^2), D], [B, HW/(4^2), D], (H/4, W/4). encoder-stage2
            ...
            if i in self.out_indices:
                ...
                out = x_out.view(-1, H // (2 ** i), W // (2 ** i), self.num_features[i]).permute(0, 3, 1, 2).contiguous()
                # print(out.shape)
                outs.append(out)
