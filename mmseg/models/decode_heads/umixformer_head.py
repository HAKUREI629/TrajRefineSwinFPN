...
class APFormerHead(BaseDecodeHead):
    ...
    """
    def __init__(self, feature_strides, decoder_params, pool_scales=(1, 2, 3, 6), **kwargs):
...
class APFormerHead2_rebuttal(BaseDecodeHead): #near stage only for Rebuttal (i.e. Defense)
    ...
    def __init__(self, feature_strides, pool_scales=(1, 2, 3, 6), **kwargs):
        ...
        self.attn_c4 = Block(dim1=c4_in_channels, dim2=c4_in_channels + c3_in_channels,                  num_heads=num_heads[0], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=8)
        self.attn_c3 = Block(dim1=c3_in_channels, dim2=c4_in_channels + c3_in_channels + c2_in_channels, num_heads=num_heads[1], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=4)
        self.attn_c2 = Block(dim1=c2_in_channels, dim2=c3_in_channels + c2_in_channels + c1_in_channels, num_heads=num_heads[2], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=2)
        self.attn_c1 = Block(dim1=c1_in_channels, dim2=c2_in_channels + c1_in_channels,                  num_heads=num_heads[3], mlp_ratio=4,
                                drop_path=0.1, pool_ratio=1)
    ...
    def forward(self, inputs):
        ...
        c_key = self.cat_key1([c4, c3]) #self key + near stage
        c_key = c_key.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c4 = c4.flatten(2).transpose(1, 2)
        _c4 = self.attn_c4(c4, c_key, h4, w4, h4, w4)

        _c4 = _c4.permute(0,2,1).reshape(n, -1, h4, w4)
        c_key = self.cat_key2([_c4, c3, c2]) #self key + near stage
        c_key = c_key.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c3 = c3.flatten(2).transpose(1, 2)
        _c3 = self.attn_c3(c3, c_key, h4, w4, h3, w3)

        _c3 = _c3.permute(0,2,1).reshape(n, -1, h3, w3)
        c_key = self.cat_key3([_c3, c2, c1]) #self key + near stage
        c_key = c_key.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c2 = c2.flatten(2).transpose(1, 2)
        _c2 = self.attn_c2(c2, c_key, h4, w4, h2, w2)

        _c2 = _c2.permute(0,2,1).reshape(n, -1, h2, w2)
        c_key = self.cat_key4([_c2, c1]) #self key + near stage
        c_key = c_key.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c1 = c1.flatten(2).transpose(1, 2)
        _c1 = self.attn_c1(c1, c_key, h4, w4, h1, w1)
...
class APFormerHeadSingle(BaseDecodeHead): #single key + replacement
    ...
    def __init__(self, feature_strides, pool_scales=(1, 2, 3, 6), **kwargs):
        ...
        self.attn_c4 = Block(dim1=c4_in_channels, dim2=c4_in_channels, num_heads=8, mlp_ratio=4,
                                drop_path=0.1, pool_ratio=8)
        self.attn_c3 = Block(dim1=c3_in_channels, dim2=c4_in_channels, num_heads=5, mlp_ratio=4,
                                drop_path=0.1, pool_ratio=4)
        self.attn_c2 = Block(dim1=c2_in_channels, dim2=c3_in_channels, num_heads=2, mlp_ratio=4,
                                drop_path=0.1, pool_ratio=2)
        self.attn_c1 = Block(dim1=c1_in_channels, dim2=c2_in_channels, num_heads=1, mlp_ratio=4,
                                drop_path=0.1, pool_ratio=1)
    ...
    def forward(self, inputs):
        ...
        c_key = c_key.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c4 = c4.flatten(2).transpose(1, 2)
        _c4 = self.attn_c4(c4, c_key, h4, w4, h4, w4) #self-attention

        _c4 = _c4.permute(0,2,1).reshape(n, -1, h4, w4)
        c_key = self.cat_key2(_c4)
        c_key = _c4.flatten(2).transpose(1, 2) #shape: [batch, h1*w1, channels]
        c3 = c3.flatten(2).transpose(1, 2)
        _c3 = self.attn_c3(c3, c_key, h4, w4, h3, w3)
