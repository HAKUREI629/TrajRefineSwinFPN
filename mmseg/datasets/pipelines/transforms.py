...
class Corruptions(object):
    ...
    def __init__(self, prob=0.5, mode='test', typ='gaussian_noise', sev=2):
        self.prob = prob
        self.version = 'v1.0'
        self.mode = mode
        self.typ = typ
    ...
    def __call__(self, results):
        ...
        # apply
        cor = True if np.random.rand() < self.prob else False
        C = len(self.corruption_cats['all'])
        typ = self.corruption_cats['all'][random.randint(0, C-1)]
        sev = random.randint(1, 5)
        if self.sev > 0 and cor:
            results['img'] = corrupt(results['img'], corruption_name=typ, severity=sev)
